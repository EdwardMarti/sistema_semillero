...
function iniciarTabla() {
    //tabla de alumnos
    $("#listadoFacultadesTabla").DataTable({
        responsive: true,
        ordering: true,
        paging: true,
        searching: true,
        ...
        rowCallback: function(row, data, index) {
            var id_facultad = data.id

            $(".actualizar", row).click(function() {
                gestionarItem(id_facultad, data, index);
            });
            $(".eliminar", row).click(function() {
                DeleteOrder(id_facultad, index);
            });
        },
...
function obtenerDatos() {
    ...
    fetch("../../back/controller/FacultadControllerList.php", {
        ...
        .then(function(data) {
            console.log(data)
            listadoFaculdates(data.falcultades);
        })
...
function listadoFaculdates(facultades) {

    let tabla = $("#listadoFacultadesTabla").DataTable();
    tabla.data().clear();
    tabla.rows.add(facultades).draw();
}
...
function mostrarModalFacultades() {
    $('#myModalFacultades').modal({ show: true });
    $("#btnOrderReg").show();
    $("#btnOrderAct").hide();
}

function cerrarModalFacultades() {

    $('#myModalFacultades').modal('hide');
...
function registrarFact() {
    if ($('#fact_titulo').val() == "" || $('#fact_titulo').val() == null) {
        Mensaje.mostrarMsjError(
            "Error",
            "Titulo de la Facultad vacio."
        );
        return false;
    }
    let fact = {
        titulo: $('#fact_titulo').val(),
    };

    Utilitario.agregarMascara();
    fetch("../../back/controller/FacultadControllerInsert.php", {
            method: "POST",
            headers: {
                Accept: "application/json",
                "Content-Type": "application/json",
...
function DeleteOrder(id) {
    Mensaje.mostrarMsjConfirmacion(
        'Eliminar Facultad',
        'Este proceso es irreversible , ¿esta seguro que desea eliminar la Facultad?',
        function() {
            eliminarFacultad(id);
        }
    );
...
function eliminarFacultad(id) {
    ...
    Utilitario.agregarMascara();
    fetch("../../back/controller/FacultadControllerDelete.php", {
            method: "POST",
            headers: {
                Accept: "application/json",
                "Content-Type": "application/json",
