...
function cargarSelectgrupos_investigacion() {
    ...
    fetch("../../back/controller/Grupo_investigacionController_List.php", {
        ...
        .then(function(data) {
            construirSelectgrupo_investigacion(data.grupo_investigacion);
        })
        .catch(function(promise) {
            if (promise.json) {
                promise.json().then(function(response) {
 ...
 */
function construirSelectgrupo_investigacion(grupo_investigacion) {
    $("#grupo_investigacion").empty();
    let input = $("#grupo_investigacion");
    for (let index = 0; index < grupo_investigacion.length; index++) {
        let gpI = grupo_investigacion[index],
